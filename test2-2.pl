prog(tmp, 
     ...
     ensures(forall([decl(a0,int), decl(p1,int), decl(p2,int)], 
     ...
     seq([for(_,P,ps,r,true,
              atomic(seq([pre(P,and([ref(t,P)>0,
                                     ref(ready,P)=0,
                                     ref(decided,P)=0,
                                     ref(ho,P)=0,
                                     ref(k,P)=0,
                          ...
                          assume(P,forall([decl(i,int)],
                                          and([ref(t,i)>0,
                                               ...
                                               ref(k,i)=0])))]))),
          for(_,A,as,r,true,
              atomic(seq([pre(A,ref(wT,A)=0),
                          assign(A,max,0),
                          assign(A,wT,0)]))),

          sym(P,ps,
              seq([for(P,A,as,_,true,
                       seq([atomic(
                                seq([pre(P,forall([decl(i,int)],
                                     ...
                                     assign(P,id,P),
                                     assign(A,pair(msgType,pair(pID,pair(pt,px))),
                                            P,pair(0,pair(id,pair(t,0)))),
                                     ite(A,ref(msgType,A)=0,
                                         seq([assign(A,success,0),
                                              ite(A,ref(pt,A)>ref(max,A),
                                                  seq([assign(A,max,ref(pt,A)),
                                                       assign(A,success,1)]),
                                                  skip)]),
                                         skip),
                                     ite(A,ref(msgType,A)=1,
                                         seq([assign(A,success,0),
                                              ite(A,ref(max,A)=<ref(pt,A),
                                                  seq([assign(A,wT,ref(pt,A)),
                                                       assign(A,w,ref(px,A)),
                                                       assign(A,success,1)]),
                                                  skip)]),
                                         skip),
                                     assign(P,pair(rwT,pair(rw,rsuccess)),
                                            A,pair(wT,pair(w,success))),
                                     ite(P,ref(rsuccess,P)=1,
                                         seq([assign(P,ho,ref(ho,P)+1)]),
                                         skip),
                                     ite(P,ref(xT,P)=<ref(rwT,P),
                                         seq([assign(P,xT,ref(rwT,P)),
                                              assign(P,x,ref(rw,P))]),
                                         skip)]))])),
                   ite(P,2*ref(ho,P)>card(as),
                       seq([pre(P,forall([decl(i,int)],implies(and([elem(i,ps),
                                                                    ref(decided,i)=1]),
                                                               and([ref(k,i)>card(as)/2,
                                                                    ref(ho,i)>card(as)/2,
                                                                    ref(ready,i)=1])))),
                            pre(P,forall([decl(i,int)],implies(and([elem(i,ps),
                                                                    here(i)]),
                                                               and([ref(decided,i)=0,
                                                                    ref(k,i)=0,
                                                                    ref(k,i)+ref(l,i)+ref(m,i)=card(as)])))),
                            ...
                            for(P,A,as,_,true,
                                seq([atomic(
                                         seq([assume(P,forall([decl(i,int)],
                                              ...
                                              assume(P,forall([decl(i,int)],
                                                              implies(ref(t,i)=ref(t,P),
                                                                      i=P))),
                                              assume(P,implies(and([0<ref(xT,P)]),
                                                               and([ref(x,P)=ref(w,a0),
                                                                    ref(xT,P)=ref(wT,a0)]))),
                                              assign(A,pair(msgType,pair(pID,pair(pt,px))),
                                                     P,pair(1,pair(id,pair(t,x)))),
                                              ite(A,ref(msgType,A)=0,
                                                  seq([assign(A,success,0),
                                                       ite(A,ref(pt,A)>ref(max,A),
                                                           seq([assign(A,max,ref(pt,A)),
