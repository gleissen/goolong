...
var dochaipaxos *bool = flag.Bool("chai", false, "Use Chaipaxos as the replication protocol. Defaults to false.")
...
func main() {
	...
	if *doEpaxos {
		...
		log.Println("Starting Chaipaxos replica...")
		rep := chaipaxos.NewReplica(replicaId, nodeList, *thrifty, *exec, *dreply, *durable)
		rpc.Register(rep)
	} else {
