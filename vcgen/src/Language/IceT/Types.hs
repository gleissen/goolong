...
import Data.Map.Strict as M hiding (foldl', filter)
...
data Rel = Eq | Le | Lt | Gt | SetMem | SetSub
         deriving (Eq, Show)
...
simplifyProp (a :=>: b) = case (a', b') of
  ...
  where
    a' = simplifyProp a
    b' = simplifyProp b
simplifyProp (Forall bs p) = case p' of
                               Forall bs2 p2 -> Forall (nub $ bs ++ bs2) p2 
                               _             -> Forall bs p'
  where
    p' = simplifyProp p
...
simplifyExpr = go
  where
    go (PExpr p)     = PExpr (simplifyProp p)
