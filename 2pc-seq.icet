prog(tmp, 
     ...
     ensures(and([forall([decl(i,int)],implies(and([elem(i,dbs),
                                                    committed=1]),
                                               ref(value,i)=proposal)),
                  forall([decl(i,int)],implies(and([elem(i,dbs),
                                                    committed=0]),
                                               ref(value,i)=0))])),

     seq([assign(c,committed,0),
          assign(c,abort,0),
          ...
          for(c,A,dbs,rr,
              ...
              seq([assign(A,value,0),
                   assign(A,val,c,proposal),
                   assign(c,vote,A,myVote),
                   ite(c,vote=0,
                       seq([assign(c,abort,1)]),
                       skip)])),
              ...
              forall([decl(i,int)],
                     and([implies(and([elem(i,rr),
                          ...
                          implies(and([elem(i,dbs),
                                       committed=0]),
                                  ref(value,i)=0)])),
              seq([assign(A,decision,c,reply),
                   ite(A,ref(decision,A)=1,
                       seq([assign(A,value,ref(val,A))]),
                       skip),
                   assign(A,ackMsg,1),
                   assign(c,ack,A,ackMsg)]))])).
